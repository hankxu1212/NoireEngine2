...
float perlin(float x, float y) {
	int X = static_cast<int>(floor(x)) & (PERLIN_SIZE - 1);
	int Y = static_cast<int>(floor(y)) & (PERLIN_SIZE - 1);
	x -= floor(x);
	y -= floor(y);
	float u = fade(x);
	float v = fade(y);
	int A = (X + Y * PERLIN_SIZE) & 255;
	int B = (X + 1 + Y * PERLIN_SIZE) & 255;
	int AA = (A + 1) & 255;
	int AB = (A + PERLIN_SIZE) & 255;
	int BA = (B + 1) & 255;
	int BB = (B + PERLIN_SIZE) & 255;

	float res = lerp(v, lerp(u, grad(AA, x, y), grad(BA, x - 1, y)),
		lerp(u, grad(AB, x, y - 1), grad(BB, x - 1, y - 1)));
	return (res + 1.0f) * 0.5f; // Normalize to 0.0 to 1.0
}
...
Tutorial::Tutorial(RTG &rtg_) : rtg(rtg_) {
	...
	std::cout << "Found Depth Format: ----------- " << string_VkFormat(depth_format) << std::endl;
	...
	{ //TODO: make some textures
		...
		{ //texture 1 will be a classic 'xor' texture:
			...
			for (uint32_t y = 0; y < size; ++y) {
				for (uint32_t x = 0; x < size; ++x) {
					float nx = static_cast<float>(x) / 2;
					float ny = static_cast<float>(y) / 2;
...
void Tutorial::update(float dt) {
	...
	{ //camera orbiting the origin:
		float ang = float(M_PI) * 2.0f * 10.0f * (time / 100.0f);
	...
	{ //make some crossing lines at different depths:
		...
		for (uint32_t i = 0; i < N; ++i) {
			...
			lines_vertices.emplace_back(PosColVertex{
				.Position{.x = std::sin(x), .y = -1.0f, .z = std::cos(z)},
				.Color{.r = 0x44, .g = 0x00, .b = 0xff, .a = 0xff},
				});
			lines_vertices.emplace_back(PosColVertex{
				.Position{.x = std::sin(x), .y = 1.0f, .z = z},
				.Color{.r = 0x44, .g = 0x00, .b = 0xff, .a = 0xff},
				});
		}
