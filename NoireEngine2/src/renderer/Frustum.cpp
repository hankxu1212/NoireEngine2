...
{
    std::copy(&other.frustum[0][0], &other.frustum[0][0] + 24, &frustum[0][0]);
}
...
bool Frustum::PointInFrustum(const glm::vec3& position) const {
...
bool Frustum::SphereInFrustum(const glm::vec3& position, float radius) const {
    for (uint32_t i = 0; i < 6; i++) {
