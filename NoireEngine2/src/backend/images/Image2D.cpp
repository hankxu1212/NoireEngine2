...
Image2D::Image2D(uint32_t w, uint32_t h, VkFormat format, VkImageLayout layout, VkImageUsageFlags usage, bool mipmap, VkSampleCountFlagBits samples) :
	Image(VK_FILTER_LINEAR, VK_SAMPLER_ADDRESS_MODE_REPEAT, samples, layout,
...
void Image2D::Load(std::unique_ptr<Bitmap> loadBitmap, bool useSampler, bool useView) {
