...
RTG::RTG(Configuration const &configuration_) : helpers(*this) {
	...
	{ //create the logical device:
		...
		VK(vkCreateDevice(physical_device, &instance_create_info, nullptr, &device));
	...
	for (auto &workspace : workspaces) {
		{ //create workspace fences:
			VkFenceCreateInfo sx_create_info{
				.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO,
				.flags = VK_FENCE_CREATE_SIGNALED_BIT, //start signaled, because all workspaces are available to start
			};

			VK(vkCreateFence(device, &sx_create_info, nullptr, &workspace.workspace_available));
		}

		{ //create workspace semaphores:
			VkSemaphoreCreateInfo sm_create_info{
				.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO,
			};

			VK(vkCreateSemaphore(device, &sm_create_info, nullptr, &workspace.image_available));
			VK(vkCreateSemaphore(device, &sm_create_info, nullptr, &workspace.image_done));
		}
	}
