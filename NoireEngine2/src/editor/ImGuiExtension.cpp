...
bool ImGuiExt::DrawVec2(float& x, float& y, const char* name, const char** labels, float columnWidth, float sensitivity) {
	bool valueChange = false;
	PushID(name);
	{
		Columns(2);
...
bool ImGuiExt::DrawVec2(glm::vec2& v, const char* name, const char** labels, float columnWidth, float sensitivity) {
...
void ImGuiExt::InspectTexture(const char** imguiIDs, const char* label, const char* path, int* id, float* multiplier, float max)
{
	...
	if (multiplier) {
		...
		ImGui::NextColumn();
		ImGui::DragFloat("##multiplier", multiplier, 0.01f, 0.0f, max, "%.2f", ImGuiSliderFlags_AlwaysClamp);
		ImGui::Columns(1);
		ImGui::PopID();
	}
}
