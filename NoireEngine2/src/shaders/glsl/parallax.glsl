vec3 ParallaxOcclusionMapping(vec2 uv, vec3 V)
{
    float viewCorrection = (-V.z) + 2.0;

	float currLayerDepth = 0.0;
	vec2 dUV = V.xy * material.heightScale / (viewCorrection * parallax_numLayers);
	float height = 1.0 - textureLod(textures[material.displacementTexId], uv, 0.0).r;
	for (int i = 0; i <parallax_numLayers; i++) {
		currLayerDepth += parallax_layerDepth;
		uv -= dUV;
		height = 1.0 - textureLod(textures[material.displacementTexId], uv, 0.0).r;
